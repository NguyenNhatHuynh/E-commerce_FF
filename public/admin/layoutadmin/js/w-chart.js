...
const datadoanhthugroupbylinechart = document.querySelector("[datadoanhthugroupbylinechart]")
let sanphamdoanhthucaonhat = []
let datalinechartsorder = []
let datalinechartrevenue = []
let titleperformancetop5product = []
let dataperformancetop5product = []
if(datadoanhthugroupbylinechart)
{
    const newdata = datadoanhthugroupbylinechart.getAttribute("datadoanhthugroupbylinechart")
    datalinechartrevenue = JSON.parse(newdata).map((item) => {
      return item.total
    })
}
...
if(datadoanhthugroupbylinechart){
  const newdata = datadoanhthugroupbylinechart.getAttribute("datadoanhthugroupbylinechart")
  datalinechartsorder = JSON.parse(newdata).map((item) => {
     return item.donhang
  })
  
}

const datatop5doanhthu = document.querySelector("[datatop5doanhthu]")
if(datatop5doanhthu){
  const newdata = datatop5doanhthu.getAttribute("datatop5doanhthu")
    titleperformancetop5product = JSON.parse(newdata).map((item) => {
    return (
       item.title
    )
  })
  dataperformancetop5product= JSON.parse(newdata).map((item) => {
      return (
        item.count
      )
    })
    sanphamdoanhthucaonhat = JSON.parse(newdata).map((item) => {
      console.log(item)
      return item.total
    })
}
...
function floatchart() {
  ...
  (function () {
    ...
    var chart = new ApexCharts(document.querySelector('#thongkehoadonshop'), options);
    chart.render();
    ...
    var options_monthly_report = {
      series: [
        {
          name: 'Sản phẩm có doanh thu cao nhất',
          data: sanphamdoanhthucaonhat
        },
      ...
      xaxis: {
        categories: titleperformancetop5product
      },
      legend: {
        show: false
      }
    ...
    var options_performance = {
      ...
      series: dataperformancetop5product,
      colors: ['#1DE9B6', '#0EACF5', '#9C92D5', '#F4C22B',"#7E8083"],
      labels: titleperformancetop5product,
      fill: {
        opacity: [1, 1, 1, 0.3]
      },
      legend: {
  ...
  // toanh tien giao dichh tren san cua san
    let totaltrasactionandregister = []
    const datatotalrevenuadmin = document.querySelector("[datatotalrevenuadmin]")
    if(datatotalrevenuadmin){
      const newdata = datatotalrevenuadmin.getAttribute("datatotalrevenuadmin")
      totaltrasactionandregister = JSON.parse(newdata).map((item) => {
      return (
        item.total
      )
    })
    ...
    var optionsadmin = {
      ...
      series: [
        {
          data: totaltrasactionandregister
        }
      ],
      xaxis: {
        categories: ['T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'T8', 'T9', 'T10', 'T11', 'T12'],
    ...
    var chart = new ApexCharts(document.querySelector('#admin-totaltrasationandrevenu'), optionsadmin);
    chart.render();
    
/// tong doanh thu cua san

    let totalrevenueofadmin = []
    if(datatotalrevenuadmin){
      const newdata = datatotalrevenuadmin.getAttribute("datatotalrevenuadmin")
      totalrevenueofadmin = JSON.parse(newdata).map((item) => {
      return (
        item.totalrevenue
      )
    })
    }
    var optiontotalrevenuofadmin = {
      chart: {
        type: 'area',
        height: 230,
        toolbar: {
    ...
    var chart = new ApexCharts(document.querySelector('#admin-totalrevenuofadmin'), optiontotalrevenuofadmin);
    chart.render();
    

    let totalrevenueofinvoiceadmin = []
    if(datatotalrevenuadmin){
      const newdata = datatotalrevenuadmin.getAttribute("datatotalrevenuadmin")
      totalrevenueofinvoiceadmin = JSON.parse(newdata).map((item) => {
      return (
        item.revenu
      )
    })
    }
    var optiontotalinvoiceofadmin = {
      chart: {
        type: 'area',
        height: 230,
        toolbar: {
      ...
      series: [
        {
          data: totalrevenueofinvoiceadmin
        }
      ],
      xaxis: {
        categories: ['T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'T8', 'T9', 'T10', 'T11', 'T12'],
    ...
    var chart = new ApexCharts(document.querySelector('#admin-totalrevenueofinvoiceadmin'), optiontotalinvoiceofadmin);
    chart.render();




    let totalrevenueofregistereadmin = []
    if(datatotalrevenuadmin){
      const newdata = datatotalrevenuadmin.getAttribute("datatotalrevenuadmin")
      totalrevenueofregistereadmin = JSON.parse(newdata).map((item) => {
      return (
        item.register
      )
    })
    }
    var optiontotalregisterofadmin = {
      chart: {
        type: 'area',
        height: 230,
        toolbar: {
      ...
      series: [
        {
          data: totalrevenueofregistereadmin
        }
      ],
      xaxis: {
        categories: ['T1', 'T2', 'T3', 'T4', 'T5', 'T6', 'T7', 'T8', 'T9', 'T10', 'T11', 'T12'],
    ...
    var chart = new ApexCharts(document.querySelector('#admin-totalrevenueofregistereadmin'), optiontotalregisterofadmin);
    chart.render();
